...
type vxLan struct {
	...
	connections   []v1.Connection // clusterID -> remote ep connection
	mutex         sync.Mutex
...
func (v *vxLan) ConnectToEndpoint(remoteEndpoint types.SubmarinerEndpoint) (string, error) {
	...
	klog.Infof("The remote endpoint is %#v", remoteEndpoint.Spec)
	...
	klog.Infof("Remote VtepIP for Ip %v is %v", ip, remoteVtepIP)

	if err != nil {
		return ip, fmt.Errorf("failed to derive the vxlan vtepIP for %s, %v", ip, err)
	}

	klog.Infof("Adding FDB for: %s", remoteIP)
	v.vxLanIface.AddFDB(remoteIP, "00:00:00:00:00:00")
	klog.Infof("Adding Route for: %s", allowedIPs)
	klog.Infof("The  VtepIP  is %v", v.vxLanIface.vtepIP)
	v.vxLanIface.AddRoute(allowedIPs, remoteVtepIP, v.vxLanIface.vtepIP)
...
func (v *vxLan) DisconnectFromEndpoint(remoteEndpoint types.SubmarinerEndpoint) error {
	...
	if err != nil {
		return fmt.Errorf("failed to derive the vxlan vtepIP for %s, %v", ip, err)
	}

	v.vxLanIface.DelFDB(remoteVtepIP, "00:00:00:00:00:00")
	v.vxLanIface.DelRoute(allowedIPs)
...
func (v *vxLan) createVxLANInterface(activeEndPoint string,) error {
	...
	klog.Infof("defaultHostIface Ip is %#v", defaultHostIface)
...
func createVxLanIface(iface *vxLanIface) error {
	...
	if err == syscall.EEXIST {
		...
		if err != nil {
			return fmt.Errorf("failed to retrieve link info1: %v", err)
		}
...
func (iface *vxLanIface) AddFDB(ipAddress net.IP, hwAddr string) error {
	klog.Infof("The Vxlan interface is %#v", iface)
...
func (iface *vxLanIface) AddRoute(ipAddressList []net.IPNet, gwIp net.IP, ips net.IP) error {
	for _,ipAddress := range ipAddressList{
		...
		if err != nil {
			...
			klog.Infof("unable to add the route entry %v, err: %s", route, err)
		} else {
			klog.Infof("Successfully added the route entry %v and gw ip %v", route, gwIp)
		}
	}
...
func (iface *vxLanIface) DelRoute(ipAddressList []net.IPNet) error {
	for _,ipAddress := range ipAddressList{
		...
		if err != nil {
			...
			klog.Infof("Successfully added the route entry %v", route)
		}
	}
