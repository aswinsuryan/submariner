...
func (c *ConnectionHandler) initClients(ctx context.Context, newOVSDBClient NewOVSDBClientFn) error {
	...
	c.nbdb, err = c.createLibovsdbClient(c.ipFamily, ctx, nbdbModel, newOVSDBClient)
	if err != nil {
...
func (c *ConnectionHandler) createLibovsdbClient(family net.IPFamily, ctx context.Context, dbModel model.ClientDBModel, newClient NewOVSDBClientFn,
	...
	dbAddress, err := discoverOvnKubernetesNetwork(family, ctx, c.k8sClientset, c.dynamicClient, zoneName)
	if err != nil {
...
func discoverOvnKubernetesNetwork(family net.IPFamily, ctx context.Context, k8sClientset clientset.Interface,
...
func discoverKindOvnKubernetesNetwork(family net.IPFamily, ctx context.Context, k8sClientSet clientset.Interface, zoneName string) (string, error) {
...
func discoverKindOvnNodeClusterNetwork(family net.IPFamily, ctx context.Context, k8sClientset clientset.Interface,
...
func createClusterNetworkWithEndpoints(family net.IPFamily, endPoints []corev1.Endpoints, zoneName string) (string, error) {
	for _, ep := range endPoints {
		if !strings.Contains(ep.Name, zoneName) {
			continue
		}
		...
		for _, subset := range ep.Subsets {
			for _, addr := range subset.Addresses {
				ip := addr.IP
				isMatch := (family == net.IPv6 && net.IsIPv6String(ip)) || (family == net.IPv4 && net.IsIPv4String(ip))
				if !isMatch {
					continue
				}
